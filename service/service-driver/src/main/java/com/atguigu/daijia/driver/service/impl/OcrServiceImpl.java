...
public class OcrServiceImpl implements OcrService {
    ...
    public IdCardOcrVo idCardOcr(MultipartFile file) {
        ...
        try{
            ...
            //3.创建身份证识别返回对象,返回的resp是一个IDCardOCRResponse的实例，与请求对象对应
            ...
            if (StringUtils.hasText(resp.getName())) {
                ...
                //上传身份证正面图片到腾讯云cos
                CosUploadVo cosUploadVo = cosService.upload(file, "idCard"); //同时会有回显地址
    ...
    public DriverLicenseOcrVo driverLicenseOcr(MultipartFile file) {
        try{
            ...
            //2 创建一个请求对象,每个接口都会对应一个request对象
            DriverLicenseOCRRequest req = new DriverLicenseOCRRequest();
            req.setImageBase64(fileBase64);

            //3 返回的resp是一个DriverLicenseOCRResponse的实例，与请求对象对应
            ...
            if (StringUtils.hasText(resp.getName())) {
                ...
                driverLicenseOcrVo.setDriverLicenseNo(resp.getCardCode());//驾驶证号
                ...
                driverLicenseOcrVo.setDriverLicenseExpire(DateTimeFormat.forPattern("yyyy-MM-dd").parseDateTime(resp.getEndDate()).toDate());//有效期限
