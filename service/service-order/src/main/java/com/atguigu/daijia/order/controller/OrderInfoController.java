...
public class OrderInfoController {
    ...
    //客户端和司机端都需要调用，前端页面两秒钟远程调用一次此方法，刷新订单状态
    ...
    @Operation(summary = "根据订单id获取订单信息") //此接口司机端和乘客端都需要调用
    @GetMapping("/getOrderInfo/{orderId}")
    public Result<OrderInfo> getOrderInfo(@PathVariable Long orderId) {
        //直接调用IService进行MybatisPlus的查询数据库
    ...
    //从数据库中获取订单数
    @Operation(summary = "根据时间段获取司机的订单数")
